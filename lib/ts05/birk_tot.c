...
void birk_tot(double X, double Y, double Z, double XKAPPA1, double XKAPPA2,
    ...
    static const double SH11[86] = {
        46488.84663, -15541.95244, -23210.09824, -32625.03856,
    ...
    static const double SH12[86] = {
        210260.4816, -1443587.401, -1468919.281, 281939.2993,
    ...
    static const double SH21[86] = {
        162294.6224, 503885.1125, -27057.67122, -531450.1339,
    ...
    static const double SH22[86] = {
        -131287.8986, -631927.6885, -318797.4173, 616785.8782,
    ...
    birk_1n2(1, 1, X, Y, Z, XKAPPA1, &FX11, &FY11, &FZ11);
    birk_shl(SH11, X_SC, X, Y, Z, &HX11, &HY11, &HZ11);
    *BX11 = FX11 + HX11;
    *BY11 = FY11 + HY11;
    *BZ11 = FZ11 + HZ11;

    double FX12, FY12, FZ12, HX12, HY12, HZ12;
    birk_1n2(1, 2, X, Y, Z, XKAPPA1, &FX12, &FY12, &FZ12);
    birk_shl(SH12, X_SC, X, Y, Z, &HX12, &HY12, &HZ12);
    *BX12 = FX12 + HX12;
    *BY12 = FY12 + HY12;
    *BZ12 = FZ12 + HZ12;
    ...
    birk_1n2(2, 1, X, Y, Z, XKAPPA2, &FX21, &FY21, &FZ21);
    birk_shl(SH21, X_SC, X, Y, Z, &HX21, &HY21, &HZ21);
    *BX21 = FX21 + HX21;
    *BY21 = FY21 + HY21;
    *BZ21 = FZ21 + HZ21;

    double FX22, FY22, FZ22, HX22, HY22, HZ22;
    birk_1n2(2, 2, X, Y, Z, XKAPPA2, &FX22, &FY22, &FZ22);
    birk_shl(SH22, X_SC, X, Y, Z, &HX22, &HY22, &HZ22);
    *BX22 = FX22 + HX22;
    *BY22 = FY22 + HY22;
    *BZ22 = FZ22 + HZ22;
}
...
void birk_1n2(int NUMB, int MODE, double X, double Y, double Z, double XKAPPA, double *BX, double *BY, double *BZ) {
    static const double A11[31] = {
        .1618068350, -.1797957553, 2.999642482, -.9322708978,
    ...
    static const double A12[31] = {
        .7058026940, -.2845938535, 5.715471266, -2.472820880,
    ...
    static const double A21[31] = {
        .1278764024, -.2320034273, 1.805623266, -32.37241440,
    ...
    static const double A22[31] = {
        .4036015198, -.3302974212, 2.827730930, -45.44405830,
    ...
    const double Xsc2 = Xsc*Xsc;
    const double Zsc2 = Zsc*Zsc;
    ...
    const double RHO_02_RHO2 = 49. + RHO2;
    const double BRACK = DPHI + 0.5*49./50.*(RHO2 - 1.) / RHO_02_RHO2;

    const double PHIS = PHI - BRACK*sin(PHI);
    const double DPHISPHI = 1. - BRACK*cos(PHI);    
    const double DPHISRHO = -2.*0.5*49.*RHO/ (RHO_02_RHO2*RHO_02_RHO2)*sin(PHI);
    ...
    const double BPHI_S = (BPHIAS - RHO*(BRHOAS*DPHISRHO))*XKAPPA;
...
void twocones(const double *A, double X, double Y, double Z, int MODE, double DTHETA, double *BX, double *BY, double *BZ) {
    ...
    double BXS, BYS, BZS;
    one_cone(A, X, -Y, -Z, MODE, DTHETA, &BXS, &BYS, &BZS);
...
void one_cone(const double *A, double X, double Y, double Z, int MODE, double DTHETA, double *BX, double *BY, double *BZ) {
    ...
    const double R_DR_PLUS = R + 1E-6;
    const double R_DR_MINUS = R - 1E-6;
    const double DRSDR = (r_s(A, R_DR_PLUS, THETA) - r_s(A, R_DR_MINUS, THETA))/2E-6;

    const double THETA_DT_PLUS = THETA + 1E-6;
    const double THETA_DT_MINUS = THETA - 1E-6;
    const double DRSDT = (r_s(A, R, THETA_DT_PLUS) - r_s(A, R, THETA_DT_MINUS))/2E-6;

    const double DTSDR = (theta_s(A, R_DR_PLUS, THETA) - theta_s(A, R_DR_MINUS, THETA))/2E-6;
    const double DTSDT = (theta_s(A, R, THETA_DT_PLUS) - theta_s(A, R, THETA_DT_MINUS))/2E-6;
...
/* TODO: caste konverzie typov double -> double */
double r_s(const double *A, double R, double THETA) {
    const double R2 = R*R;
    const double R2_AA = R2 + (double)A[15]*(double)A[15];
    return R + A[1] / R + A[2]*R/ sqrt(R2 + ((double) A[10]*(double)A[10])) + A[3]*R/(R2 + ((double)A[11]*(double)A[11])) +
           (A[4] + A[5] / R + A[6]*R/sqrt(R2 + ((double)A[12]*(double)A[12])) + A[7]*R/(R2 + ((double)A[13]*(double)A[13])))*cos(THETA) +
           (A[8]*R/sqrt(R2 + ((double)A[14]*(double)A[14])) + A[9]*R/(R2_AA*R2_AA))*cos(2.*THETA);
}

double theta_s(const double *A, double R, double THETA) {
    const double R2 = R*R;
    return THETA +
           (A[16] + A[17] / R + A[18]/ R2 + A[19]*R/sqrt(R2 + (double)A[26]*(double)A[26]))*sin(THETA) +
           (A[20] + A[21]*R/sqrt(R2 + (double)A[27]*(double)A[27]) + A[22]*R/(R2 + (double)A[28]*(double)A[28]))*sin(2.*THETA) +
           (A[23] + A[24] / R + A[25]*R/(R2 + (double)A[29]*(double)A[29]))*sin(3.*THETA);
}

// TODO: polia su nepotrebne, rovnako cyklus for, na konci sa vyberu vysledky len z indexu MODE.
...
void birk_shl(const double *A, double X_SC, double X, double Y, double Z, double *BX, double *BY, double *BZ) {
    ...
    for(int j = 0; j < 3; j++) {
        ...
        for(int k = 0; k < 3; k++) {
            ...
            GX += (-SQPR*EPR*CYPI*SZRK) * (A[L] + X_SC * A[L + 1] + A[L + 2] + X_SC * A[L + 3]);
            GY += (EPR*SYPI*SZRK/P) * (A[L] + X_SC * A[L + 1] + A[L + 2] + X_SC * A[L + 3]);
            GZ += (-EPR*CYPI*CZRK/R) * (A[L] + X_SC * A[L + 1] + A[L + 2] + X_SC * A[L + 3]);
            L += 4;
        }
    }
